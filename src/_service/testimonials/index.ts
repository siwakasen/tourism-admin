...
export const useCreateTestimonial = async (data: CreateTestimonialReqI) => {
    try {
        ...
        const res = await axios.post(`${process.env.REACT_APP_REST_TESTI}/testimonials`, formData, {
            headers: {
...
export const useUpdateTestimonial = async (data: UpdateTestimonialReqI) => {
    try {
        const formData = new FormData();
        ...
        const res = await axios.post(`${process.env.REACT_APP_REST_TESTI}/testimonials/${data.id}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
                'Authorization': `Bearer ${data.access_token}`
            }, });
